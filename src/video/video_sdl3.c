/** @file src/video/video_sdl3.c SDL 3 video driver. */

/*
 * based on src/video/video_sdl2.c
 */
...
static int s_screen_magnification = 2;
...
static void Video_Mouse_Move(int32 x, int32 y)
{
	...
	x /= s_screen_magnification;
	y /= s_screen_magnification;
...
void Video_Mouse_SetPosition(uint16 x, uint16 y)
{
	 ...
	 * exactly. Note that the values from SDL_GetCurrentRenderOutputSize are in
	 * physical units while SDL_GetRenderViewport are in logical units.
	 */
	if (!SDL_GetRenderViewport(s_renderer, &rect)) {
		Error("SDL_GetRenderViewport failed: %s\n", SDL_GetError());
		return;
	}

	if (!SDL_GetCurrentRenderOutputSize(s_renderer, &w, &h)) {
		Error("SDL_GetCurrentRenderOutputSize failed: %s\n", SDL_GetError());
		return;
	}
...
*/
int Video_CreateWindowAndRenderer(int width, int height, Uint32 window_flags,
...
bool Video_Init(int screen_magnification, VideoScaleFilter filter)
{
	...
	renderer_name = (IniFile_GetInteger("renderer_software_fallback", 0) != 0) ? SDL_SOFTWARE_RENDERER : NULL;
	...
	err = SDL_SetRenderLogicalPresentation(s_renderer,
					render_width,
					render_height,
...
static void Video_DrawScreen_Nearest_Neighbor(void)
{
	...
	if (!s_screen_needrepaint && area && (area->left > 0 || area->top > 0 || area->right < SCREEN_WIDTH || area->bottom < SCREEN_HEIGHT)) {
		...
		for (y = area->top; y < area->bottom; y++) {
			...
			gfx_screen8 += SCREEN_WIDTH - (int)rect.w;
			pixels += pitch;
		}
		Debug("Dirty area NN : (%d,%d)-(%d,%d)\n", area->left, area->top, area->right, area->bottom);
	} else {
	...
	if (!SDL_RenderTexture(s_renderer, s_texture, prect, prect)) {
		Error("SDL_RenderTexture failed : %s\n", SDL_GetError());
	}
}
...
static void Video_DrawScreen_Hqx(void)
{
	...
	SDL_UnlockTexture(s_texture);
	if (!SDL_RenderTexture(s_renderer, s_texture, NULL, NULL)) {
		Error("SDL_RenderTexture failed : %s\n", SDL_GetError());
	}
}
	...
	while (SDL_PollEvent(&event)) {
		...
		switch (event.type) {
			...
			case SDL_EVENT_KEY_UP:
			{
				...
				uint8 code = 0;
				if ((key == SDLK_RETURN && (mod & SDL_KMOD_ALT)) || key == SDLK_F11) {
					/* ALT-ENTER was pressed */
					if (keyup) continue;	/* ignore key-up */
					if (!SDL_SetWindowFullscreen(s_window, s_full_screen ? 0 : SDL_WINDOW_FULLSCREEN)) {
				...
				if (key == SDLK_RSHIFT) {
					...
					key -= SDLK_CAPSLOCK;
					if (key < sizeof(s_SDL_hikeymap)) code = s_SDL_hikeymap[key];
				} else {
					if (key < sizeof(s_SDL_keymap)) code = s_SDL_keymap[key];
				}
				if (code == 0) {
					if (scancode == SDL_SCANCODE_GRAVE) {
						...
						Warning("Unhandled key scancode=0x%X key=0x%X %s\n",
						        scancode, key,
						        SDL_GetKeyName(key));
						continue;
					}
